...
export default function Start() {
    ...
    const {IMAGES, isMobale, startGame} = useContext(Context);
            ...
            <div className="start-paralax" onMouseMove={({ clientX: x, clientY: y }) => set({ xy: calc(x, y) })}>
                ...
                <div className="start-app-content">
                    ...
                    <div className="start-app-content-paragraph">
                        <span>
                            Отличная вечеринка — это когда гости танцуют, поют и общаются под любимую музыку. Веселая и дружеская атмосфера вдохновляет на то, чтобы взять в руки микрофон и устроить настоящий караоке-батл! Если твоя вечеринка проходит с JBL, <a href="https://bit.ly/2V0cLAM" target="_blank" rel="noreferrer">выпустившим новую акустическую систему JBL PartyBox On-the-Go</a> с беспроводным микрофоном, то у тебя больше нет причин стесняться своих талантов. Готов поразить всех своим бесподобным выступлением? Исполняй треки — а мы расскажем, получилось ли у тебя устроить незабываемый вечер для своих гостей.
                        </span>
                    </div>
                    <div className="start-app-content-button app-btn app-btn-red" onClick={startGame}>
                        <span>Зажигаем!</span>
                    </div>
                </div>
            </div>
